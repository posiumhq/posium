...
import { ID_PATTERN } from "../types/context.js";
import type { BrowserAgentPage } from "../BrowserAgentPage.js";
import type { LogLine } from "../types/log.js";
import {
  ContentFrameNotFoundError,
  BrowserAgentDomProcessError,
  BrowserAgentElementNotFoundError,
  BrowserAgentIframeError,
  XPathResolutionError,
} from "../types/browserAgentErrors.js";
...
const WORLD_NAME = "browser-agent-world";
 ...
 * @param sp - The BrowserAgentPage wrapper for Playwright and CDP calls.
...
export async function buildBackendIdMaps(
  ...
  if (!targetFrame || targetFrame === (sp as any).page.mainFrame()) {
    session = await (sp as any).getCDPClient();
  } else {
    try {
      session = await (sp as any).context.newCDPSession(targetFrame); // OOPIF
    } catch {
      session = await (sp as any).getCDPClient(); // same-proc iframe
    }
  }

  await (sp as any).enableCDP(
    "DOM",
    session === (await (sp as any).getCDPClient()) ? undefined : targetFrame,
  );
  ...
  try {
    ...
    if (
      targetFrame &&
      targetFrame !== (sp as any).page.mainFrame() &&
      session === (await (sp as any).getCDPClient())
    ) {
      // same-proc iframe: walk down to its contentDocument
      const frameId = rootFid!;
      const { backendNodeId } = await (sp as any).sendCDP<{
    ...
    while (stack.length) {
      ...
      const enc = (sp as any).encodeWithFrameId(fid, node.backendNodeId);
 ...
 *
 * @param sp - The BrowserAgentPage instance for issuing CDP commands.
...
export async function getCDPFrameId(
  ...
  if (!frame || frame === (sp as any).page.mainFrame()) return undefined;

  // 1. Same-proc search in the page-session tree
  const rootResp = (await (sp as any).sendCDP("Page.getFrameTree")) as unknown;
  ...
  try {
    const sess = await (sp as any).context.newCDPSession(frame); // throws if detached
    ...
    throw new BrowserAgentIframeError(targetUrl, String(err));
  }
}
 ...
 *
 * @param agentPage - The BrowserAgentPage instance for Playwright and CDP interaction.
...
export async function getAccessibilityTree(
  ...
  await (agentPage as any).enableCDP("Accessibility", targetFrame);
  ...
  try {
    ...
    if (
      targetFrame &&
      targetFrame !== (agentPage as any).page.mainFrame()
    ) {
      // try opening a CDP session: succeeds only for OOPIFs
      let isOopif = true;
      try {
        await (agentPage as any).context.newCDPSession(targetFrame);
      } catch {
        isOopif = false;
      }
      ...
      if (!isOopif) {
        ...
        const frameId = await getCDPFrameId(agentPage, targetFrame);
        logger({
    ...
    const { nodes: fullNodes } = await (agentPage as any).sendCDP<{
      nodes: AXNode[];
    ...
    await (agentPage as any).disableCDP("Accessibility", targetFrame);
  }
}
 ...
 *
 * @param page - The BrowserAgentPage instance for issuing CDP commands.
...
export async function getFrameRootBackendNodeId(
  ...
  if (!frame || frame === (sp as any).page.mainFrame()) {
    return null;
  }

  // Create a CDP session on the main page context
  const cdp = await (sp as any).page.context().newCDPSession((sp as any).page);
...
export async function getAccessibilityTreeWithFrames(
  ...
  const main = (agentPage as any).page.mainFrame();
  ...
  while (frameStack.length) {
    ...
    try {
      ...
      const backendId =
        frame === main
          ? null
          : await getFrameRootBackendNodeId(agentPage, frame);
      ...
      const frameId = await getCDPFrameId(agentPage, frame);
  ...
  for (const { backendNodeId, frameId, tree } of snapshots)
    if (backendNodeId !== null && frameId !== undefined)
      ...
      idToTree.set(
        (agentPage as any).encodeWithFrameId(frameId, backendNodeId),
        tree,
      );
 ...
 *
 * @param agentPage - A BrowserAgentPage instance with built-in CDP helpers.
...
export async function findScrollableElementIds(
  ...
  const xpaths: string[] = targetFrame
    ...
    : await (agentPage as any).page.evaluate(() =>
        (window as any).getScrollableElementXpaths(),
      );
  ...
  for (const xpath of xpaths) {
    ...
    if (objectId) {
      const { node } = await (agentPage as any).sendCDP<{
        node?: { backendNodeId?: number };
      }>("DOM.describeNode", { objectId }, targetFrame);
 ...
 *
 * @param page     A BrowserAgentPage (or Playwright.Page with .sendCDP)
...
export async function resolveObjectIdForXPath(
  ...
  const { result } = await (page as any).sendCDP<{
  ...
  if (!result?.objectId) throw new BrowserAgentElementNotFoundError([xpath]);
...
async function getFrameExecutionContextId(
  ...
  if (!frame || frame === (agentPage as any).page.mainFrame()) {
    // Main frame (or no frame): use the default world.
    return undefined;
  }
  const frameId: string = (await getCDPFrameId(agentPage, frame))!;
  const { executionContextId } = await (agentPage as any).sendCDP<{
 ...
 *
 * @param sp - The BrowserAgentPage instance for evaluating XPath and locating frames.
  ...
  while (true) {
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      if (IFRAME_STEP_RE.test(steps[i])) {
        ...
        const handle = (ctxFrame ?? (sp as any).page.mainFrame()).locator(
          selector,
        );
        const frame = await handle
          .elementHandle()
          .then((h: any) => h?.contentFrame());
