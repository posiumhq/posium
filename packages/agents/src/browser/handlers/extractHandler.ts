...
import type { TextAnnotation } from "../types/textannotation.js";
import { extract } from "../inference/index.js";
import type { LLMClient } from "../../llm/LLMClient.js";
import { formatText } from "../../utils/index.js";
...
/**
 * The `BrowserAgentExtractHandler` class is responsible for extracting structured data from a webpage.
...
export class BrowserAgentExtractHandler {
  ...
  private async textExtract<T extends z.ZodObject<z.ZodRawShape>>({
    instruction,
    schema,
    content = {} as z.infer<T>,
    llmClient,
    ...
    const originalDOM = await this.agentPage.page.evaluate(() =>
      window.storeDOM(),
    );

    // **3:** Process the DOM to generate a selector map of candidate elements
    const domProcessResult = (await this.browserAgent.page.evaluate(() =>
      window.processAllOfDom(),
    )) as { selectorMap: Record<number, string[]> };
    const { selectorMap } = domProcessResult;

    this.logger.debug(
      { category: "extraction", selectorMapSize: Object.keys(selectorMap).length },
      "received output from processAllOfDom",
    );
    ...
    await this.browserAgent.page.evaluate(() =>
      window.createTextBoundingBoxes([]),
    );
    const pageWidth = await this.browserAgent.page.evaluate(
      () => window.innerWidth,
    );
    const pageHeight = await this.browserAgent.page.evaluate(
      () => window.innerHeight,
    );
    ...
    for (const xpaths of Object.values(selectorMap)) {
      ...
      const boundingBoxes = (await this.agentPage.page.evaluate(
        // @ts-expect-error - window.getElementBoundingBoxes is not defined in the global scope
    ...
    // **7:** Restore the original DOM after mutations
    await this.agentPage.page.evaluate(
    ...
    // Cast output to the expected schema type
    const typedOutput = output as unknown as z.infer<T>;
    await this.agentPage.cleanupDomDebug();

    // **10:** Handle the extraction response and log the results
    this.logger.debug(
      { category: "extraction", extractionResponse },
  ...
  private async domExtract<T extends z.ZodObject<z.ZodRawShape>>({
    ...
    this.logger.debug(
      { category: "extraction", instruction },
      "starting extraction using old approach",
    );
    ...
    const processDomResult = (await this.browserAgent.page.evaluate(
      (chunksSeen?: number[]) => window.processDom(chunksSeen ?? []),
      chunksSeen,
    )) as { outputString: string; chunk: number; chunks: number[] };
    const { outputString, chunk, chunks } = processDomResult;

    this.logger.debug(
      {
        category: "extraction",
        chunk,
        chunksLeft: chunks.length - chunksSeen.length,
        chunksTotal: chunks.length,
      },
      "received output from processDom",
    );
